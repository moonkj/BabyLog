...
## 2-1. 데이터 보존·백업 (최우선 — 아기 사진이 핵심 자산)

로컬 전용 저장의 가장 큰 위험은 **앱 삭제 시 기기 컨테이너가 통째로 사라지는 것**이다(앱 삭제 = 사진·기록 전부 소실). 이를 막기 위한 다층 대책:

| 계층 | 동작 | 상태 |
|---|---|---|
| ① 원자적 저장 | `state.json` atomic write | 적용 |
| ② 디코딩 실패 보호 | 손상 시 `state.corrupt-*.json` 백업 + 자동저장 차단(빈 상태가 원본 덮어쓰기 방지) | 적용 |
| ③ **전체 백업 파일** | 사진·영상·기록을 단일 `.babylogbackup`(바이너리 plist)로 묶어 파일 앱/iCloud Drive/AirDrop에 저장·복원 (`BackupService`) | 적용 |
| ④ 백업 권유 | 설정에 마지막 백업일 표시, 14일↑ 미백업 시 강조 | 적용 |
| ⑤ CloudKit 자동백업 | 유료 계정 연결 시 기기 간 자동 동기화(아래) | 스캐폴드 |

- ③ 사용 위치: **설정 > 데이터 백업** — "전체 백업 내보내기"(공유 시트) / "백업에서 복원"(파일 선택). 새 기기·재설치 후 복원하면 사진까지 되살아난다.
- 권장 안내(앱 내 고지): "앱을 삭제하면 기기 데이터가 사라져요. 정기적으로 백업하거나 iCloud 가족 백업을 켜세요."
- 자동화 로드맵: 유료 계정 연결 후 ⑤ CloudKit으로 무중단 자동 백업 → 사용자 수동 백업 부담 제거.
