...
## 세션 — 실데이터 복구 · 리팩토링 · UX 고도화 · QA (완료 · 2026-06-10)

라운드 11~13 통합 이후, "기능이 동작하지 않는다" 피드백을 받아 **목업 셸 → 실데이터 CRUD** 전면 연결 후 리팩토링·모션 고도화·QA를 수행. 매 단계 시뮬레이터 그린 + 실기기 설치 + 푸시.

- **기능 동작 복구**: 가계부(지출 추가/삭제/영속·실통계), 접종 완료(안정키 영속), 홈 죽은 버튼 8개(탭 이동·응급), 빠른기록 카운트, 응급 지도 연동, 크루 초대(공유시트)
- **A 버킷 실데이터화**: 프로필(기록·뱃지·아이·연속기록), 홈 타임라인(실 기록·1년 전 오늘), 임신(태동 일일 영속·체중 차트)
- **AppStore 코어 확장**: Expense·vaccineCompletions·PregnancyLog 영속, `objectWillChange` 기반 autoPersist(확장성), 하위호환 디코딩
- **리팩토링**: 1000줄급 화면 4개 → 섹션 서브뷰 분해(루트 합 3702→971줄)
- **UX/UI 고도화**: DESIGN §8 시그니처 모션 4계층 + 햅틱 + 검증 흔들림 (reduce motion 존중·transform만·화면당 1모션)
- **QA 패스**: 임신 모드 홈 죽은 버튼 5곳, 프로필 Pro 안내, 크루 섹션 액션 정리
- 테스트 214 → **239** (+25 회귀 방지)
- 백엔드/키 연동 절차는 [`docs/integration-backend.md`](integration-backend.md) 참조
