...
마지막 갱신: 2026-06-10 (Phase 0 확인 통합) · 갱신자: lead
...
- **[T2] 원칙 충돌: 성별 중립 vs 뱃지명 '골든 맘'** — qa 발견. SPEC 7.2 뱃지명('골든 맘/믿음직한 맘')은 SPEC B.5 '성별 중립(맘/파파 선택·중립 뱃지)'·CLAUDE.md 절대원칙과 충돌. 팀장 견해: **절대원칙 우선 → 중립 명칭 재정의 권장.** 최종 네이밍은 오너(A2).
...
### A. 즉시 결정 필요 — 오너(제품) 판단 [원칙 층위]
- **A1.** 무광고 절대원칙 vs v3 '브랜드 배너 광고(직계약)' — 직계약 배너 노출 허용 여부 (v1 비차단)
- **A2.** 뱃지명 '골든 맘' 등 → 성별 중립 명칭으로 재정의 승인 여부
...
### D. 계산 컨벤션 — 팀장 기본값 제안, 오너 확인
- **D1.** 임신 주수: LMP 기준 산출, 초음파 보정 EDD 입력 시 EDD 우선 / 'D+주차' 표기
- **D2.** 월령: Calendar 기반 개월수(생후 N개월) + D+N일 병기
- **D3.** '기록 멈춤': (CLAUDE.md 해석) 사용자 토글 강제 X, 미접속 자동 감지로 권유 알림 '자동 억제' + 따뜻한 수동 진입점. 임계값 후보 미접속 14일
