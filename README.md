...
> **아이의 모든 순간을 담다** — 임신 확인 순간부터 아이가 자라는 매일까지, 기록·동네·거래·가계부를 하나의 앱에 담은 iOS 육아 슈퍼앱.

> 📖 **제품 전체 개요(as-built)는 [`docs/OVERVIEW.md`](docs/OVERVIEW.md)** 를 보세요 — 소개·기능·아키텍처·등급·로드맵을 한 문서에 정리했습니다. 이 README는 저장소 진입(가치·스택·빌드) 요약입니다.
...
| 사진 서버 최소화 | 아동 기록 원본(성장·일기·접종·가계부·성장 보드)은 로컬/개인 iCloud(서버 X). 우리 서버(R2)는 **가족 사진 공유**에 한해 사용 — 무료 부부 2명 / Pro 8명. |
...
| 로컬/백업 | JSON 디스크(App Group) + CloudKit 개인 iCloud 백업 (CoreData 마이그레이션은 후속) |
| 백엔드 | Supabase (Postgres · Auth · Storage · Realtime) + Cloudflare R2 — 가족 피드 v1 라이브 |
| 결제 | StoreKit 2 |
