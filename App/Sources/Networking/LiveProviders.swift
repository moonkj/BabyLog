...
enum HospitalResponseParser {
    ...
    private static func areaCode(forSido nm: String?) -> String? {
        ...
        let map: [String: String] = [
            ...
            "제주": "064",
            // 결합형 풀네임(충청북도 등)은 prefix "충북"과 어긋나므로 정확매칭 키를 별도 등록.
            "충청북도": "043", "충청남도": "041", "전라북도": "063", "전라남도": "061",
            "경상북도": "054", "경상남도": "055", "경기도": "031",
            "전북특별자치도": "063", "강원특별자치도": "033", "제주특별자치도": "064",
        ]
        if let exact = map[nm] { return exact }
        // "서울특별시"·"부산광역시"처럼 광역시/특별시 풀네임은 앞 2글자 prefix로 매칭
